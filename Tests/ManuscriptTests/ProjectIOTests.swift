...
final class ProjectIOTests: XCTestCase {
    ...
    func testOpenProjectLateLockFileWriteDoesNotPreventOpen() throws {
        ...
        XCTAssertNotNil(opener.currentProject)
        XCTAssertEqual(opener.projectRootURL, root)
        XCTAssertNoThrow(try opener.saveManifest())
    }

    func testCreateProjectLateLockFileWriteDoesNotPreventCreate() throws {
        let manager = FileSystemProjectManager()
        ...
        XCTAssertNoThrow(try manager.createProject(name: "CreateLateLockFailure", at: tempDir))
        XCTAssertNotNil(manager.currentProject)
        XCTAssertEqual(manager.projectRootURL, tempDir.appendingPathComponent("CreateLateLockFailure"))
        XCTAssertNoThrow(try manager.saveManifest())
    }
    ...
    func testBackupRestoreRejectsSymlinkDestinationOutsideSourceProject() throws {
        let manager = FileSystemProjectManager()
        _ = try manager.createProject(name: "BackupSymlinkOutside", at: tempDir)
        let root = tempDir.appendingPathComponent("BackupSymlinkOutside")
        let backup = try BackupManager.createBackup(projectURL: root, retentionCount: 10)

        let externalTarget = tempDir.appendingPathComponent("external-restore-target", isDirectory: true)
        try FileManager.default.createDirectory(at: externalTarget, withIntermediateDirectories: true)
        let symlinkDestination = tempDir.appendingPathComponent("restore-link-external", isDirectory: true)
        try? FileManager.default.removeItem(at: symlinkDestination)
        try FileManager.default.createSymbolicLink(at: symlinkDestination, withDestinationURL: externalTarget)
        ...
        XCTAssertThrowsError(
            ...
            XCTAssertTrue(message.contains("cannot be a symbolic link"))
        }

        let unexpectedRestoreRoot = externalTarget.appendingPathComponent("BackupSymlinkOutside", isDirectory: true)
        XCTAssertFalse(FileManager.default.fileExists(atPath: unexpectedRestoreRoot.path))
    }

    func testBackupRestoreRejectsDestinationWhenPathIsFile() throws {
        let manager = FileSystemProjectManager()
        _ = try manager.createProject(name: "BackupDestinationFile", at: tempDir)
        let root = tempDir.appendingPathComponent("BackupDestinationFile")
        let backup = try BackupManager.createBackup(projectURL: root, retentionCount: 10)
        let sourceManifest = root.appendingPathComponent("manifest.json")
        let sourceManifestData = try Data(contentsOf: sourceManifest)

        let fileDestination = tempDir.appendingPathComponent("restore-file-destination")
        try "not a directory".write(to: fileDestination, atomically: true, encoding: .utf8)

        XCTAssertThrowsError(
            try BackupManager.restoreBackup(projectURL: root, backupFilename: backup.filename, to: fileDestination)
        ) { error in
            guard case let ProjectIOError.backupNotFound(message) = error else {
                return XCTFail("Expected backupNotFound, got \(error)")
            }
            XCTAssertTrue(message.contains("must be a directory"))
        }
    ...
    func testBackupRestoreRejectsArchiveMissingManifest() throws {
        ...
        let root = tempDir.appendingPathComponent("BackupMissingManifest")
        let backupsDir = root.appendingPathComponent("backups", isDirectory: true)
        let fakeName = "backup-missing-manifest.zip"
        let fakeZipURL = backupsDir.appendingPathComponent(fakeName)

        let stageRoot = tempDir.appendingPathComponent("fake-backup-stage-\(UUID().uuidString)", isDirectory: true)
        let stagedProject = stageRoot.appendingPathComponent("BackupMissingManifest", isDirectory: true)
        try FileManager.default.createDirectory(at: stagedProject, withIntermediateDirectories: true)
        try "placeholder".write(
            to: stagedProject.appendingPathComponent("README.txt"),
            atomically: true,
            encoding: .utf8
        )
        ...
        let restoreDir = tempDir.appendingPathComponent("restore-missing-manifest-temp", isDirectory: true)
        XCTAssertThrowsError(
